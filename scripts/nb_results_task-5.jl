...
c = read_results!("../data/cluster_task-5") ;

# ╔═╡ 8e4c9d81-1c82-482a-aa55-ad2d904c5274
c_raw = collect_results!("../data/cluster_task-5") ;

# ╔═╡ 63090b83-6d91-4e77-82ac-a21196a460c8
md"## Skewed results"

# ╔═╡ fefe9c2f-4773-477d-be15-f64eeb164b6f
md"I had a bug in the skewed results where I had a mix between skewed + normal errors, thus making them less skewed. This is fixed & tested now. The new results, including one-sided tests are as follows"
...
let
	...
	d= data(c|>c->@rsubset(c,
		...
		:nSubject == 30,
		:β == [0.,0.],
		:side  == Symbol("twosided"),
		ismissing(:residualMethod) || :residualMethod !="shuffle",
		:coefname == "condition: B",
	)) * mapping(
		#(:statsMethod,:nItemsPerCondition)=>(x,y)->x*" | items: "*@sprintf("%02i",y),
		(:imbalance,:side) =>(x,y)->x*" - "*string(y),#:imbalance,
		:pval,
...
# ╔═╡ d08efa24-fe1f-4bc8-b7df-9165a12b4a30
md"""
With a keen eye, you can spot that "balanced" (xvalue 1) and "imbalance between conditions" (x-value 3) are identical patterns. 

ergo let's drop "condition/trialwise"

"""

# ╔═╡ af9f2e2d-693d-4c51-8df6-b8afaafae085
let
...
# ╔═╡ bda0fb4d-9a09-4cda-b8ae-30ba1e9497d3
md"""
ok, ok... still too complex. Notice how **pBoot** and **waldsT** have almost the same pattern, with **pBoot** being a bit better?

Let's remove **waldsT** for now
"""

# ╔═╡ b4552d3d-293a-4985-9d0f-116ef9ab58e8
let
...
# ╔═╡ cb6286b2-0eaf-4593-b788-b9e4517f32d7
md"""
Ah, perfection.
We can conclude that permutation test controls the type-1 better than pboot/waldsT - not much difference between 

Let's add complications now!

## Shuffle or Flip Residuals
!!! hypothesis
	we thought **flipping residuals** should **not** be legitimate for skewed residuals
"""

# ╔═╡ 42b57aa5-3fb5-4864-bb0e-af99d6cf37fc
let
...
# ╔═╡ 83b71357-8570-401f-b897-9708d651a5e7
md"""
!!! conclusion
	Shuffle or flipping matterns for **skewed** **imbalanced** results - surprisingly, flipping seems better?

## lesser/greater/twosided testing?
we were thinking, that one-sided testing should influence how our skewed results look like.
"""

# ╔═╡ 33e3d71a-6921-4e4d-a2d0-1b4e2a954af8
let
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "8e6c18e4e65f1266d76264bbf13695a7cf9252a5"
