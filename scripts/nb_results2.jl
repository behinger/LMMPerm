...
# ╔═╡ d2b98375-f6dd-4385-be32-2296dda88b84
include("../src/sim_results.jl")

# ╔═╡ 97a6b2d2-ae77-447d-b760-208dc54e5c57
c_raw = collect_results!("../data/cluster_sim")


# ╔═╡ 5992de8a-561e-4cb3-9104-4536c8b9ca1b
...
md"""
## Load and preprocess c
"""
end

# ╔═╡ 1ce93685-4c07-48b0-8156-97fe610c8609
...
md"""
Select dataset: $(@bind ix PlutoUI.Select(1:size(c,1)))
Select Beta $(@bind select_beta PlutoUI.Select(unique(c.β)))
...
let
	...
	d= data(c|>c->@rsubset(c,:β == select_beta,
							:coefname == "condition: B",
							:statsMethod == "permutation",
							ismissing(:errorDistribution)
				) |> 
	c->@transform(c,@byrow :pval=(select_beta==[0,0])&&(:pval>0.1) ? 0.1 : :pval))	* mapping(
		:f_simple=>sorter(["1|s","1+a|s","1+a|s + 1+a|i"]),
		:pval,
		#color=:blupMethod,
		color = :residualMethod,
		marker=:inflationMethod,
		col=:σs_simple=>sorter(["1|s","1+1*a|s","4+1*a|s","1+4*a|s","1+1*a|s+1|i","1+1*a|s+1+1*a|i","1+1*a|s+1+4*a|i"]),
		row = :test,
	)*visual(Scatter)|>x->draw(x,legend=(position=:bottom,),axis=(xticklabelrotation=45.,))
...
# ╔═╡ 9a493157-689b-4779-b82f-602279d7a365
c

# ╔═╡ 03375296-0974-449c-8a1f-2019e5e4a0fb
let
	c.trueDiff = [x[2] for x in c.β]
	d= data(c|>c->@rsubset(c,:f_simple == "1+a|s",
		:σs_simple == "1+1*a|s",
		:test != "β",
		...
		:trueDiff,#:β,
		:pval,
		marker=:nSubject=>nonnumeric,
		color = :statsMethod,
		#col=:blupMethod,
		col = :errorDistribution,
		row = :nSubject=>nonnumeric,
...
manifest_format = "2.0"
project_hash = "e1d457d532a8d70e1c4b9b4ab5c064234ebd80c6"
