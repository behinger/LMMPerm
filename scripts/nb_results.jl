...
let
	...
	d= data(c|>c->@rsubset(c,:β == select_beta,ismissing(:statsMethod))|> c->@transform(c,@byrow :p=(select_beta==[0,0])&&(:p>0.1) ? 0.1 : :p)) * mapping(
	...
	#θs=$(res.σs[1])"
	#xtickrotation!!(d.figure,45)
	#ylims!(current_axis(),0.0,0.08)
	d
end
